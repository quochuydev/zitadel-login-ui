...
function createUserService(accessToken: string) {
  ...
  return {
    ...
    updateProfile: async (params: {
      ...
      // Read the current user so unchanged fields are preserved and
      // unchanged endpoints are skipped (Zitadel rejects no-op updates).
      const { user } = await zitadelService.request<UserByID>({
        url: 'v2beta/users/{userId}',
        method: 'get',
        headers,
        params: {
      ...
      const profileChanged =
        profile?.givenName !== firstName || profile?.familyName !== lastName;

      if (profileChanged) {
        await zitadelService.request<UpdateHumanProfile>({
      ...
      if (username && username !== user?.username) {
        await zitadelService.request<UpdateUserName>({
